...
export function PlanGoalSelector({ planSlug, values, onChange }: PlanGoalSelectorProps) {
  ...
  const tiersToShow = useMemo(() => {
    return PLAN_GOAL_TIER_ORDER.map((tier) => ({
      tier,
      meta: PLAN_GOAL_TIER_META[tier],
      goals: PLAN_GOALS_BY_TIER[tier],
      unlocked: planTierUnlocked(tier, currentTier),
    }))
  }, [currentTier])
  ...
  return (
    <div>
      ...
      <p style={{ ...s.stepHint, marginBottom: spacing[4] }}>
        Choose at least {PLAN_GOAL_MIN} and up to {PLAN_GOAL_MAX} goals. Goals outside your plan stay locked —
        upgrade anytime to unlock them.
      </p>

      {tiersToShow.map(({ tier, meta, goals }) => {
        const unlocked = planTierUnlocked(tier, currentTier)
        return (
          <section key={tier} style={{ marginBottom: spacing[4] }}>
            <div
              style={{
                display: 'flex',
                alignItems: 'center',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                >
                  {meta.title}
                  <span style={{ fontWeight: 500, color: colors.textMuted }}> · {meta.shortLabel}</span>
                </h3>
              </div>
              <span
                style={{
              ...
              >
                {unlocked ? 'Included' : 'Locked'}
              </span>
            </div>

            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {goals.map((goal) => {
                const selected = values.includes(goal.value)
                const locked = !isGoalUnlockedForPlan(goal, currentTier)
                const blockedByMax = !selected && atMax && !locked
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: '8px 0 0', fontSize: 14, lineHeight: 1.5, color: colors.textSecondary }}>
        ...
        {currentMeta.title} plan includes earlier goals — upgrade to add this one to your selection.
      </p>
