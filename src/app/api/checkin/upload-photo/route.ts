...
import { CHECKIN_PHOTO_BUCKET } from '@/lib/checkin'
import {
...
const LABEL_RE = /^[a-z0-9_]{1,40}$/i

/**
 * Same-origin check-in photo upload fallback.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { error } = await admin.storage.from(CHECKIN_PHOTO_BUCKET).upload(path, bytes, {
      upsert: false,
