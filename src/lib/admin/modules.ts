export type AdminModuleStatus = 'implemented' | 'placeholder'

export type AdminModule = {
  id: string
  title: string
  description: string
  href: string
  status: AdminModuleStatus
  showInNav: boolean
  navOrder: number
  /** Primary database tables used by this module */
  tables: string[]
}

/** Canonical registry of admin console modules. Implement one module at a time. */
export const ADMIN_MODULES: AdminModule[] = [
  {
    id: 'dashboard',
    title: 'Dashboard',
    description: 'Financial P&L, customers, enrollment, and platform health.',
    href: '/admin/dashboard',
    status: 'implemented',
    showInNav: true,
    navOrder: 10,
    tables: ['profiles', 'purchases', 'ai_generation_logs', 'redemption_codes'],
  },
  {
    id: 'clients',
    title: 'Clients',
    description: 'Client roster, search, filters, coach assignment, and membership extensions.',
    href: '/admin/clients',
    status: 'implemented',
    showInNav: true,
    navOrder: 20,
    tables: ['profiles', 'coaches'],
  },
  {
    id: 'coaches',
    title: 'Coaches',
    description: 'Coach roster and workload overview.',
    href: '/admin/coaches',
    status: 'implemented',
    showInNav: true,
    navOrder: 30,
    tables: ['coaches', 'profiles'],
  },
  {
    id: 'purchases',
    title: 'Purchases',
    description: 'Razorpay payments, refunds, and subscription status.',
    href: '/admin/purchases',
    status: 'implemented',
    showInNav: true,
    navOrder: 40,
    tables: ['purchases'],
  },
  {
    id: 'redemption-codes',
    title: 'Enrollment Codes',
    description: 'Exact codes for old/offline members with membership expiry dates.',
    href: '/admin/redemption-codes',
    status: 'implemented',
    showInNav: true,
    navOrder: 50,
    tables: ['redemption_codes', 'redemption_usages'],
  },
  {
    id: 'promo-codes',
    title: 'Discount & Referral Codes',
    description: 'Create checkout discount and referral codes that reduce payment amount.',
    href: '/admin/promo-codes',
    status: 'implemented',
    showInNav: true,
    navOrder: 55,
    tables: ['promo_codes', 'promo_code_usages'],
  },
  {
    id: 'onboarding',
    title: 'Pending Onboarding',
    description: 'Clients who have not completed onboarding.',
    href: '/admin/onboarding',
    status: 'implemented',
    showInNav: true,
    navOrder: 60,
    tables: ['profiles'],
  },
  {
    id: 'plans',
    title: 'Active Plans',
    description: 'Read-only view of delivered coaching plans.',
    href: '/admin/plans',
    status: 'implemented',
    showInNav: true,
    navOrder: 70,
    tables: ['plans'],
  },
  {
    id: 'plan-endings',
    title: 'Renewal Calls',
    description: 'Expired memberships and upcoming endings, with contact details for renewal calls.',
    href: '/admin/plan-endings',
    status: 'implemented',
    showInNav: true,
    navOrder: 75,
    tables: ['profiles', 'purchases'],
  },
  {
    id: 'expected-enrollments',
    title: 'Expected enrollments',
    description: 'Open consult leads by expected call day, before they enroll.',
    href: '/admin/expected-enrollments',
    status: 'implemented',
    showInNav: true,
    navOrder: 77,
    tables: ['talk_to_coach_submissions', 'profiles', 'purchases'],
  },
  {
    id: 'transformation-scores',
    title: 'Transformations',
    description: 'Ranked client transformation scores with links to profiles.',
    href: '/admin/transformation-scores',
    status: 'implemented',
    showInNav: true,
    navOrder: 32,
    tables: ['profiles', 'checkins'],
  },
  {
    id: 'support',
    title: 'Support Queue',
    description: 'Oversight of client coaching support requests.',
    href: '/admin/support',
    status: 'implemented',
    showInNav: true,
    navOrder: 80,
    tables: ['support_requests', 'support_messages'],
  },
  {
    id: 'issues',
    title: 'Feedback',
    description: 'Client reviews, complaints, and bug reports.',
    href: '/admin/issues',
    status: 'implemented',
    showInNav: true,
    navOrder: 90,
    tables: ['issue_reports'],
  },
  {
    id: 'prompts',
    title: 'Prompt Library',
    description: 'Manage AI coaching prompts and version history.',
    href: '/admin/prompts',
    status: 'implemented',
    showInNav: true,
    navOrder: 100,
    tables: ['prompt_library', 'prompt_library_versions'],
  },
  {
    id: 'ai-logs',
    title: 'AI Logs',
    description: 'AI generation trace logs for debugging and QA.',
    href: '/admin/ai-logs',
    status: 'implemented',
    showInNav: true,
    navOrder: 110,
    tables: ['ai_generation_logs'],
  },
  {
    id: 'notifications',
    title: 'Notifications',
    description: 'Platform notification templates and delivery logs.',
    href: '/admin/notifications',
    status: 'implemented',
    showInNav: true,
    navOrder: 120,
    tables: ['platform_notifications', 'user_notifications'],
  },
  {
    id: 'settings',
    title: 'Settings',
    description: 'System configuration and feature flags.',
    href: '/admin/settings',
    status: 'implemented',
    showInNav: false,
    navOrder: 200,
    tables: [],
  },
  {
    id: 'testing-tools',
    title: 'Testing Tools',
    description: 'Create trial coach and client accounts for internal QA and demos.',
    href: '/admin/testing-tools',
    status: 'implemented',
    showInNav: false,
    navOrder: 210,
    tables: ['profiles', 'coaches'],
  },
]

export function getAdminNavModules(): AdminModule[] {
  return ADMIN_MODULES.filter((m) => m.showInNav).sort((a, b) => a.navOrder - b.navOrder)
}

export function getAdminModuleById(id: string): AdminModule | undefined {
  return ADMIN_MODULES.find((m) => m.id === id)
}

export function getAdminModuleByHref(href: string): AdminModule | undefined {
  return ADMIN_MODULES.find((m) => m.href === href || href.startsWith(`${m.href}/`))
}
