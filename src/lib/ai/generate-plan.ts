...
const LIBRARY_WORKOUT_OUTPUT_INSTRUCTIONS = [
  ...
  '- Use "sets x reps" format with a plain letter x (e.g. "Barbell Bench Press — 4 sets x 8 reps"). Prefer "Exercise — N sets x M reps" or "Exercise: N sets x M reps".',
  '- For timed work use "Exercise — N sets x M sec" or "Exercise — M min".',
  '- CRITICAL: Label every training day clearly as Day 1, Day 2, Day 3, … (never weekday names like Monday). Cover every required training day for this client. Do not omit days or leave empty day headers.',
  '- Prefer also filling workout_plan.days with one object per training day (day, focus, exercises) using Day N labels.',
