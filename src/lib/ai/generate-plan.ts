...
const LIBRARY_DIET_OUTPUT_INSTRUCTIONS = [
  ...
  '- Every day under a day header (Monday, Tuesday, Day 1, etc.) must include the FULL meal list written out in actual words with portions, cooking fats, and macro lines.',
  '- NEVER use cross-day references such as "same as Monday", "repeat Tuesday", "follow Day 1", "as above", or "use Monday\'s plan". If a day intentionally mirrors another day, rewrite that day\'s complete meals again under the new day header. The daily tracker cannot resolve day-to-day pointers.',
