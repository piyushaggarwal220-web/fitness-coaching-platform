...
export async function editPlanSection(input: EditPlanSectionInput): Promise<EditPlanSectionResult> {
  ...
  const systemPrompt = [
    ...
    '- Do not use Markdown, asterisks, bullets, or symbol prefixes.',
    '- Never use a hyphen, dash, minus, or horizontal rule character in the revised plan.',
    '- Write numeric ranges with the word to and write compound words with spaces.',
