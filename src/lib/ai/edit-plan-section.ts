...
export async function editPlanSection(input: EditPlanSectionInput): Promise<EditPlanSectionResult> {
  ...
  const systemPrompt = [
    ...
    'Rules:',
    '- Preserve useful structure: day headers must stay as Day 1, Day 2, … (never convert to weekday names), meal names, exercise lines with sets x reps (plain letter x).',
