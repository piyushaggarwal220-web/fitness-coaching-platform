...
function buildCheckinSection(checkin: Checkin): string {
  const lines = [
    ...
    `- Sleep quality: ${formatValue(checkin.sleep_quality)}/10`,
    `- Stress level: ${formatValue(checkin.stress_level)}/10`,
    `- Hunger level: ${formatValue(checkin.hunger_level)}/10`,
    `- Motivation: ${formatValue(checkin.motivation_level)}/10`,
    `- Diet adherence: ${formatValue(checkin.diet_adherence)}/10`,
    `- Workout adherence: ${formatValue(checkin.workout_adherence)}/10`,
    `- Days followed diet: ${formatValue(checkin.days_followed_diet)}`,
    `- Days trained: ${formatValue(checkin.days_followed_workout)}`,
    `- Days sleep on target: ${formatValue(checkin.days_followed_sleep)}`,
    `- Days water on target: ${formatValue(checkin.days_followed_water)}`,
    `- Days steps on target: ${formatValue(checkin.days_followed_steps)}`,
    ...
    `- Pain / injuries: ${hasMeaningfulText(checkin.pain_injuries) ? checkin.pain_injuries!.trim() : 'None'}`,
