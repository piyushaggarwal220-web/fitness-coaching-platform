...
const refundTerms = read('src/lib/legal/terms-content.ts')
const refundOperations = read('src/lib/payments/admin-operations.ts')
const refundEligibility = read('src/lib/payments/refund-eligibility.ts')
...
assert.match(planProvider, /Live generation is OpenAI-only/)
assert.doesNotMatch(planProvider, /explicit === 'claude'/)
assert.match(refundPolicy, /redirect\('\/terms#payments-refunds-guarantees'\)/)
assert.match(refundTerms, /at least 90%/)
assert.match(refundTerms, /does not limit rights that cannot legally be excluded|non-excludable/)
assert.match(refundEligibility, /at least 90%/)
