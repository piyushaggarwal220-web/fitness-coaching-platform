...
assert.equal(dueSchedule.todayTasks.some(
  (task) => task.type === 'mid_week' && task.status === 'available'
), true)

const submitRoute = source('src/app/api/checkin/submit/route.ts')
assert.match(submitRoute, /if \(body\.checkinType === 'weekly'\) \{\s*after\(/)
...
assert.match(draftSource, /Mid-week check-ins require a coach reply in chat and never create a plan draft/)
...
assert.match(dashboard, /dueCheckinTask/)
assert.match(dashboard, /Check-in notification/)
...
assert.doesNotMatch(reminderCron, /alreadySentToday/)

const deliveryMigration = source(
  'supabase/migrations/20260721163500_notification_delivery_system.sql'
)
assert.match(
  deliveryMigration,
  /\('weekly_checkin_reminder', 'normal', ARRAY\['in_app','web_push'\]/
)
assert.match(
  deliveryMigration,
  /\('mid_week_checkin_reminder', 'normal', ARRAY\['in_app','web_push'\]/
)
